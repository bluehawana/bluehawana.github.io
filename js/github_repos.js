...
async function fetchLatestRepos(containerId = 'github-repos') {
    try {
        ...
        // Manually add the Company Project as the first item
        const featuredProject = {
            html_url: "https://hongyanab.com",
            name: "Hongyana Business",
            description: "A comprehensive business consulting platform bridging Swedish and Chinese markets. Features full-stack architecture optimized for high performance, SEO, and cross-border accessibility.",
            language: "Full Stack",
            stargazers_count: "Featured",
            updated_at: new Date().toISOString()
        };

        const featuredElement = document.createElement('div');
        featuredElement.className = 'repo-item featured-project';
        featuredElement.innerHTML = `
            <div class="repo-card featured" style="border: 2px solid var(--accent-primary);">
                <h4><a href="${featuredProject.html_url}" target="_blank">${featuredProject.name}</a> <span style="font-size: 0.8em; color: var(--accent-secondary);">★ Featured</span></h4>
                <p class="repo-description">${featuredProject.description}</p>
                <div class="repo-meta">
                    <span class="language">${featuredProject.language}</span>
